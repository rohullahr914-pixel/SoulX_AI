...
export async function GET(request: Request) {
  try {
    ...
    if (section === "personas") {
      const result = await query(`SELECT p.slug,p.name,p.creator_id,p.visibility,p.is_featured,p.is_trending,p.is_suspended,p.created_at,pr.full_name creator,COALESCE(s.unique_users,0) users,COALESCE(s.conversations,0) chats,COALESCE(s.likes,0) likes,COALESCE(s.followers,0) followers,COALESCE(s.rating,0) rating,COALESCE(s.trending_score,0) trending FROM personas p LEFT JOIN profiles pr ON pr.id=p.creator_id LEFT JOIN persona_stats s ON s.persona_slug=p.slug ORDER BY p.created_at DESC LIMIT 200`, []);
